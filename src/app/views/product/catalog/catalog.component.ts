...
export class CatalogComponent implements OnInit {
          ...
          .subscribe(params => {
            ...
            this.activeParams.types.forEach(url => {
              for (let i = 0; i < this.categoriesWithTypes.length; i++) {
                ...
                if (foundType) {
                  this.appliedFilters.push({name: foundType.name, urlParam: foundType.url})
                }
              }
            })
  ...
  removeAppliedFilter(appliedFilter: AppliedFilterType) {
    if (appliedFilter.urlParam === 'heightFrom' || appliedFilter.urlParam === 'heightTo'
      ...
      const {[appliedFilter.urlParam]: _, ...updatedParams} = this.activeParams;
      this.activeParams = updatedParams;
    } else {
      this.activeParams.types = this.activeParams.types.filter(item => item !== appliedFilter.urlParam);
    }

    this.activeParams.page = 1
    const queryParams = {...this.activeParams};
